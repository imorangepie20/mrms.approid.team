...
- 2026-09-26 `3ccc0d25f5ed`에서 멜론 수집을 최대 100곡씩 처리하고 후보 확인 완료 60초 후 체크포인트에서 자동 재개하도록 변경했다. 운영 `019_ems_melon_batches.sql` 적용, DB 백업 확인, Web·EMS 배포와 기존 작업 재개를 완료했다. 기존 작업의 미처리 후보를 먼저 처리 중이며 실제 다음 묶음 전환은 관찰 중이다. 상세는 `docs/changes/2026-09-26-melon-100-track-batches.md`에 기록한다.
