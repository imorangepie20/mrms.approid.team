...
- 전역 플레이어와 검색의 앨범·플레이리스트 상세는 공식 TIDAL Embed와 TIDAL 페이지 진입점을 제공한다.
- Embed를 열기 전에 SDK 재생을 일시 정지하며, 앱 큐와 셔플·반복 상태는 유지한다.
...
- 검색 상세에서 앨범·플레이리스트 저장은 지원하지 않는다.
- TIDAL Embed 내부 재생과 로그인 상태는 TIDAL이 결정하며 부모 앱 큐에서 제어하지 않는다.
- 공개 도메인의 TIDAL Embed 데스크톱·모바일 시각 검증은 아직 하지 않았다.

## 다음 작업

1. 공개 도메인에서 트랙·앨범·플레이리스트 TIDAL Embed를 확인한다.
2. 실제 모바일 기기에서 TIDAL 오디오 출력을 확인한다.
3. MMS 플레이리스트 카드의 상세 화면과 큐 재생을 연결한다.
4. 실제 플레이리스트를 가져온 뒤 MMS 데이터 상태와 트랙 재생을 확인한다.
