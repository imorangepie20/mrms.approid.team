...
멜론 한국대중음악: `018_ems_melon_genres.sql` 적용 뒤 `/admin/ems/melon`에서 수동 작업을 시작·일시정지·재개한다. 장르 탭 `GN0100`–`GN0800`의 최신곡 목록과 페이지를 순회하며 곡 ID, 곡명, 가수, 앨범, 장르, 원본 URL, 최초·최근 확인 시각을 저장한다. 목록 페이지당 최대 50곡을 처리하고 체크포인트를 남긴다. 같은 곡을 여러 장르에서 발견해도 원천곡은 하나이며 장르 관계는 모두 보존한다. 24시간 정기 루틴은 `/admin/ems/routines`의 `멜론 한국대중음악` 카드에서 제어한다. 초기 예약은 migration 적용 24시간 뒤다.

- 기본 멜론 요청 간격은 최소 3초이며 `MELON_MIN_REQUEST_INTERVAL_SECONDS`로 늘릴 수 있다. 429는 `Retry-After` 뒤 한정 재시도한다. 401·403·로그인/차단 이동·목록 구조 변경은 실패 코드로 표시하고 우회하지 않는다.
- 원천곡 저장과 EMS 활성곡은 다르다. 멜론 곡을 TIDAL에서 곡명·가수·앨범으로 확인하고 KR `STREAM`을 확인한 뒤에만 활성화한다. 모호하거나 일치하지 않는 후보는 승격하지 않는다.
- 장르별 원천 수와 TIDAL 연결 수는 관리자 페이지에서 별도로 본다. 마지막 페이지가 비거나 50곡 미만이면 다음 장르로 진행한다.
- `019_ems_melon_batches.sql`부터 목록 수집은 최대 100곡씩 진행한다. 한 묶음의 미처리 TIDAL 후보가 없어지면 60초 뒤 저장한 장르·페이지부터 다음 묶음을 자동 시작한다. 장르 마지막 페이지가 100곡 미만이면 그 지점에서 묶음을 닫는다. 배포 시 기존 열린 작업의 후보를 먼저 처리한 뒤 새 묶음을 시작한다. 관리자 화면은 현재 묶음 곡 수·확인 대기 후보·다음 묶음 시각을 표시한다.
- 배포 시 Web과 `ems-pipeline` 이미지를 함께 갱신한다. DB 백업 후 migration을 적용한다. 문제가 나면 새 워커를 이전 이미지로 되돌리고 멜론 작업을 일시정지한다. 운영 데이터를 지우는 down migration은 운영 롤백에 쓰지 않는다.
