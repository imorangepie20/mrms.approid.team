...
class CandidateSelector:
    ...
    def select(self, rows: Iterable[CanonicalRow], limit: int) -> list[Candidate]:
        ...
        unique = {row.recording_mbid: row for row in rows}
        ordered = list(unique.values())
        ordered.sort(key=lambda row: (_key(row), self.seed))
        ...
        artist_cap = max(1, int(limit * 0.001))
        release_cap = 5
        ...
        def try_add(row: CanonicalRow, bucket: str) -> bool:
            ...
            selected.append(Candidate(
                ...
                selection_score=max(0.0, min(1.0, row.canonical_score)),
            ))
        ...
        canonical = sorted(ordered, key=lambda row: (-row.canonical_score, _key(row)))
        diverse = sorted(ordered, key=lambda row: (row.artist_region or "unknown", (row.release_date or "")[:4], _key(row)))
        long_tail = sorted(ordered, key=lambda row: (row.canonical_score, -(date.fromisoformat(row.release_date).toordinal() if row.release_date else 0), _key(row)))
...
def write_candidate_artifacts(
    ...
    with gzip.open(candidates_path, "wt", newline="", encoding="utf-8") as handle:
    ...
    manifest = build_manifest(snapshot_id, candidates_path, len(candidates), selector_version, seed)
