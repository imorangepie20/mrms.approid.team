...
export async function GET(request: Request) {
  try {
    ...
    if ((cursor && params.has("page")) || (cursor && cursor.length > 256) || !Number.isInteger(page) || page < 1 || page > 100_000 || !Number.isInteger(limit) || limit < 1 || limit > 100 || query.length > 120 || (status && !TRACK_STATUSES.has(status)) || (embeddingStatus && !EMBEDDING_STATUSES.has(embeddingStatus))) {
      return Response.json({ code: "invalid_admin_track_filter" }, { status: 400 });
    }
    return Response.json(await listEmsAdminTracks({ query, page: cursor ? undefined : page, cursor, limit, status, embeddingStatus }, getDatabasePool()));
  } catch (error) {
